...
export type LayoutPlan = {
  ...
  photo?: PhotoCheck;
  /**
   * Отпечаток фото, по которому план построен. Бриф живёт на клиенте, и
   * человек может заменить фото, не нажимая «Собрать» — тогда генерация шла
   * по плану от ПРЕЖНЕГО снимка: чертёж обрабатывался как фотография, а фото
   * деталей как «графика» с пустым фоном (2026-09-21, evgenigorovoi86).
   * Сервер сверяет отпечаток при генерации и пересчитывает план сам.
   */
  photoHash?: string;
};
