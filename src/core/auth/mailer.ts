...
export async function sendVerificationEmail(to: string, code: string): Promise<void> {
  ...
  const subject = `=?UTF-8?B?${b64("Код подтверждения — WB Card Studio")}?=`;
  const body = b64(
    `Ваш код подтверждения: ${code}\r\n\r\n` +
      `Код действует 15 минут. Если вы не регистрировались в WB Card Studio, просто проигнорируйте это письмо.\r\n`,
  );
  const message =
    `From: WB Card Studio <${from}>\r\n` +
    `To: <${to}>\r\n` +
    `Subject: ${subject}\r\n` +
    `MIME-Version: 1.0\r\n` +
