...
/**
 * Вебхук публичного Telegram-бота. Telegram НЕ достаёт до kartogen.ru напрямую
 * (getWebhookInfo: «Connection timed out» — блок в обе стороны), поэтому
 * setWebhook смотрит на релей relay/api/webhook.js (Vercel), а он пересылает
 * апдейт сюда с тем же секретным заголовком. Отвечаем тоже через релей.
 *
