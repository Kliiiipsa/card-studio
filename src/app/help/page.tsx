...
const FAQ: { q: string; a: string }[] = [
  ...
  {
    q: "Чем «Рекламные креативы» отличаются от «Инфографики»?",
    a: "«Инфографика» — слайд для карточки на Wildberries или Ozon: без логотипа, цены, кнопки и контактов, потому что маркетплейсы за это снимают карточку. «Рекламные креативы» — реклама за пределами маркетплейса: баннер для Директа и VK, пост для соцсетей, шапка профиля, визитка. Там логотип, цена, телефон и сайт как раз нужны, и модель печатает их прямо в картинке. Для карточки товара — «Инфографика», для рекламы — «Креативы».",
  },
  {
  ...
  {
    q: "Можно ли вернуть деньги за неиспользованные гены?",
    a: "Да, напишите на admin@kartogen.ru с почтой аккаунта. Считаем просто: из суммы, которую вы заплатили, вычитаем стоимость услуг, которыми вы уже воспользовались, — разницу возвращаем тем же способом оплаты в течение 10 рабочих дней. Бонусные гены (бонус пакета, промокоды, приглашения) в расчёте не участвуют: денег за них не приходило, поэтому деньгами они не возвращаются. Пример: заплатили 200 ₽, получили 210 генов, потратили 50 — вернём 150 ₽. Полные условия — в пунктах 9.9–9.11 оферты.",
  },
  {
